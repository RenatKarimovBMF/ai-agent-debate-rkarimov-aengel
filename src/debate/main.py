...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="AI Agent Debate — The Godfather vs The Shawshank Redemption"
    )
    parser.add_argument(
        "--config",
        type=Path,
    ...
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Open optional topic launcher GUI (terminal mode still available)",
    )
    ...
    if args.pro and args.con and args.topic:
        ...
        config = with_custom_debate(
            config, pro_side=args.pro, con_side=args.con, topic=args.topic
        )
    if args.dry_run:
